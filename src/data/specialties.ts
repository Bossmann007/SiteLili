...
export const specialties = [
  {
    ...
    sections: [
      ...
      {
        heading: 'Como funciona na consulta',
        paragraphs: [
      ...
      {
        heading: 'Acompanhamento e acesso',
        paragraphs: [
      ...
      {
        heading: 'Ligação com outras áreas',
        paragraphs: [
...
export function getRelatedSpecialties(currentSlug: string): Specialty[] {
  const current = getSpecialty(currentSlug);
  if (!current) return [];
  return current.relatedSlugs.flatMap((slug) => {
    if (slug === currentSlug) return [];
    const related = getSpecialty(slug);
    return related ? [related] : [];
  });
}

const MEV_PILLARS_CARD_SLUGS = new Set([
  'medicina-do-estilo-de-vida',
  'prevencao',
  'longevidade',
]);

export function showsMevPillarsCard(slug: string): boolean {
  return MEV_PILLARS_CARD_SLUGS.has(slug);
}
